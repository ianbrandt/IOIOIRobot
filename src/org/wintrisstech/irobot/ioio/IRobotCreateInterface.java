...
/**
 * A high level interface to the iRobot series of Roomba/Create robots.
 * It encapsulates most of the commands and sensors specified in
 * <a href=../../../../Create%20Open%20Interface_v2.pdf>
 * Create Open Interface_v2.pdf</a>.
 * It is recommended reading that document
 * in order to get a better understanding of how to work with the Create.
 *
 */
public interface IRobotCreateInterface {

    /**
     * Identifies a demo where the Create attempts to cover an entire
     * room using a combination of behaviors, such as random bounce, wall
     * following, and spiraling.
     * @see #demo(int)
     */
     ...
     * behind. If Create hits an obstacle while driving, it drives away from
     * the obstacle.
     * @see #demo(int)
     */
     ...
     * small opening in the front of the infrared receiver with black electrical tape.
     * <p>
     ...
     *  SENSORS_CLIFF_LEFT_SIGNAL, <br>
     *  SENSORS_CLIFF_FRONT_LEFT_SIGNAL, <br>
     *  SENSORS_CLIFF_FRONT_RIGHT_SIGNAL, <br>
     *  SENSORS_CLIFF_RIGHT_SIGNAL, <br>
     ...
     *  SENSORS_REQUESTED_RIGHT_VELOCITY, <br>
    ...
    /**
     * Names for sensor ids. Sensor Id = X has name = SENSOR_NAMES[X]. The
     * names may be useful for debugging or tracing purposes.
     */
     ...
     * below. A negative velocity makes Create drive backward. 
     * Available in modes: Safe or Full <p>
    ...
    /**
     * This method lets you control the forward and backward motion of the Create
     * by specifying the velocity of each wheel independently. A
     * positive velocity makes that wheel drive forward, while a negative
     * velocity makes it drive backward. Available in modes: Safe or Full
     *
    ...
    /**
     * Gets the angle in degrees that the Create has turned from the time before last
     * that the angle sensor was read to the last time the angle sensor was read.
     ...
     * capped at its minimum or maximum, which are -32768 and 32767 
     * respectively.
     * <p>
     * <b>NOTE:</b> The Create uses wheel encoders to measure distance and angle.
     * If the wheels slip, the actual distance or angle traveled may differ from
     * the Create's measurements.
     * <p>
    ...
    /**
     * Gets the estimated charge capacity of the Create's battery in milliamphours (mAh).
     ...
     * fresh values.
     *
     * @return The estimated charge capacity of the Create's battery in milliamphours (mAh).
     * @see #readSensors(int sensorId)
     * @see #readSensors(int[] sensorIds)
     */
     ...
     * returned as a value between 0 and 4095.
     * <b>NOTE:</b> This method returns a locally stored value previously read
     * from the Create. It is the client's responsibility to read the sensor
     * values from the Create prior to calling this method in order to ensure
     * fresh values.
    ...
    /**
     * Gets the distance in mm that the Create has travelled from the time before last
     * that the distance sensor was read to the last time the distance sensor was read.
     ...
     * which are -32768 amd 32767 respectively.
     * <p>
     * <b>NOTE:</b> This method returns a locally stored value previously read
     * from the Create. It is the client's responsibility to read the sensor
     * values from the Create prior to calling this method in order to ensure
    ...
    /**
     * Gets the value of the IR byte received by the Create.
    ...
    /**
     * Gets the turn radius in mm most recently requested using the
     * drive() method.
     * <p>
     * <b>NOTE:</b> This method returns a locally stored value previously read
     * from the Create. It is the client's responsibility to read the sensor
     ...
     * and Advance LEDs is specified by true or false. Power LED on will use full
     * green to match other LEDs. Available in modes: Safe or Full <br>
    ...
    /**
     * Stops or restarts the steam without clearing the
     * list of requested packets. Available in modes: Passive, Safe, or Full
     *
...
//    public void pauseResumeSensorStream(boolean pause);
    /**
     * This command loads a script into the serial input
     * queue for playback. Available in modes: Passive, Safe, or Full.
     *
    ...
    /**
     * Selects a song to play from the songs previously added to
     * the Create using one of the song() methods. This method does nothing if
     * a song is already playing. Note that the {@link #isSongPlaying() 
     * isSongPlaying()} method can be used to check whether the Create is
     * ready to accept this command. Available in modes: Safe or Full.
     *
    ...
    /**
     * Controls the three low side drivers with variable
     ...
     * choose a duty cycle of 128 * 25% = 32. Available in modes: Safe or Full <br>
     *
    ...
    /**
     * Retrieves one or more sensor values from the Create
     * and stores the values locally. The values can later be accessed using any
     * of the access methods. The sensors that are to be read are specified
     * using a sensor id. There are 43 different sensor Ids; see the SENSORS_*
     * constants. Each identifies a specific sensor or group of sensors.
     *
     ...
     * of the access methods. The sensors that are to be read are specified
     * using a sensor id. There are 43 different sensor Ids; see the SENSORS_*
     * constants. Each sensor Id identifies a specific sensor or group of sensors.
     *
     * @param sensorIds An array of the SENSORS_* constants.
     * @throws IllegalArgumentException if the array of sensorIds is too long.
     */
    public void readSensors(int[] sensorIds) throws IllegalArgumentException;

    /**
     * Puts the OI into Safe mode, enabling user control of the Create.
     ...
     * @see #getInfraredByte()
     */
    public void sendIr(int irValue);
     ...
     *
     * @param pin0High if true sets the output of the first digital output to high
     * @param pin1High if true sets the output of the second digital output to high
     * @param pin2High if true sets the output of the third digital output to high
     */
     ...
     * @see #playScript(byte[])
     * @see <a href=../../../../Create%20Open%20Interface_v2.pdf>
     * Create Open Interface_v2.pdf</a>
     *
     */
    ...
    /**
     * This method lets you specify a song that you can play at a later time.
     * Up to 16 songs can be specified. Each song is associated with a song
     * number. Each song can contain up to sixteen notes. Each note is
     * associated with a note number (in the range of (31 - 127) that uses MIDI
     * note definitions and a duration that is specified in fractions of a
     * second.  The Create considers all musical notes 
     * outside the range of (31 - 127) as rest notes, and will make no sound 
     * during the duration of those notes. Available in modes:
     * Passive, Safe, or Full <br>
     *
     ...
     *
     * @param songNumber The song number associated with the specific song. Must
     * be in the range 0 through 15. On a second invocation of the song() method,
...
//    public void startSensorStream(int[] sensorIds);
     ...
     *
     * @param playButton If true wait for the play button, otherwise wait for the advance button
     */
